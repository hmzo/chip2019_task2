...
class Evaluator(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if _val_f1 > self._best_f1:
            ...
            K.set_value(pos_weight, 0.1)
            logger.info("Set the pos_weight to %s" % str(0.1))

        # 超过二分之一个patience没有提高，则提高pos_weight
        if self.epochs - self.best_epoch > self.patience // 2 and isinstance(pos_weight, tf.Variable):
            new_pos_weight = max(0.1,
                                 min((1 + self.epochs - self.best_epoch - self.patience // 2) * 0.1,
                                     max_pos_weight))
...
class DataGenerator:
    def __init__(self, data, batch_size=32, test=False):
        """
        data format:
            train or valid: [token_list_of q1, token_list_of q2, category, label]
            test:           [token_list_of q1, token_list_of q2, category, id   ]
        """
        self.data = data
